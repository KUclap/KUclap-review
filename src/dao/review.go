...
func (m *SessionDAO) UpdateClapById(id string, newClap uint64, updateAt time.Time) error {
	err := db.C(COLLECTION_REVIEWS).UpdateId(bson.ObjectIdHex(id), bson.M{"$inc": bson.M{"clap": newClap}, "$set": bson.M{"update_at": updateAt}})
	return err
}

// Update boo by id
func (m *SessionDAO) UpdateBooById(id string, newBoo uint64, updateAt time.Time) error {
	err := db.C(COLLECTION_REVIEWS).UpdateId(bson.ObjectIdHex(id), bson.M{"$inc": bson.M{"boo": newBoo}, "$set": bson.M{"update_at": updateAt}})
	return err
}

// Update reported
func (m *SessionDAO) UpdateReportById(id string, updateAt time.Time) error {
	err := db.C(COLLECTION_REVIEWS).UpdateId(bson.ObjectIdHex(id), bson.M{"$set": bson.M{"reported": true, "update_at": updateAt}})
	return err
}
...
func (m *SessionDAO) UpdateStatsClassByCreated(classId string, newStats models.StatClass) error {
	err := db.C(COLLECTION_CLASSES).Update(bson.M{"class_id": classId}, bson.M{"$set": bson.M{"stats": newStats}, "$inc": bson.M{"number_reviewer": 1}})
	return err
}
...
func (m *SessionDAO) UpdateNuberReviewByClassID(classId string, updateAt time.Time) error {
	err := db.C(COLLECTION_CLASSES).Update(bson.M{"class_id": classId}, bson.M{"$inc": bson.M{"number_reviewer": 1}})
	return err
}
...
func (m *SessionDAO) FindClassByClassID(classId string) (models.Class, error) {
	...
	err := db.C(COLLECTION_CLASSES).Find(bson.M{"class_id": classId}).One(&class)
	return class, err
}
...
func (m *SessionDAO) FindAllClasses() ([]models.Class, error) {
	...
	err := db.C(COLLECTION_CLASSES).Find(bson.M{}).All(&classes)
	return classes, err
}

// Insert class to database
func (m *SessionDAO) InsertClass(class models.Class) error {
	err := db.C(COLLECTION_CLASSES).Insert(&class)
	return err	
}
...
func (m *SessionDAO) FindReviewsByClassID(classId string, page string, offset string) ([]models.ResReview, error) {
	...
	err = db.C(COLLECTION_REVIEWS).Find(bson.M{"class_id": classId}).Sort("-$natural").Skip(iPage * iOffset).Limit(iOffset).All(&reviews)
	return reviews, err
}
...
func (m *SessionDAO) LastReviews(page string,offset string) ([]models.ResReview, error) {
	...
	// err = db.C(COLLECTION_REVIEWS).Find(bson.M{}).Sort("-$natural").Limit(iOffset).All(&reviews)
	err = db.C(COLLECTION_REVIEWS).Find(bson.M{}).Sort("-$natural").Skip(iPage * iOffset).Limit(iOffset).All(&reviews)
	return reviews, err
}
...
func (m *SessionDAO) FindAll() ([]models.ResReview, error) {
	var reviews []models.ResReview
	err := db.C(COLLECTION_REVIEWS).Find(bson.M{}).All(&reviews)
	return reviews, err
}
...
func (m *SessionDAO) FindById(id string) (models.Review, error) {
	...
	err := db.C(COLLECTION_REVIEWS).FindId(bson.ObjectIdHex(id)).One(&review)
	return review, err
}

// Insert a review into database
func (m *SessionDAO) Insert(review models.Review) error {
	err := db.C(COLLECTION_REVIEWS).Insert(&review)
	return err
}

// Delete an existing review
func (m *SessionDAO) DeleteById(id string) error {
	err := db.C(COLLECTION_REVIEWS).RemoveId(bson.ObjectIdHex(id))
	return err
}
...
// 	err := db.C(COLLECTION_REVIEWS).Find(bson.M{"reported": bson.M{"$ne": false}}).All(&reviews)
// 	return reviews, err
// }

// Update an existing review
// func (m *SessionDAO) Update(review models.Review) error {
// 	err := db.C(COLLECTION_REVIEWS).UpdateId(review.ID, &review)
// 	return err
// }
